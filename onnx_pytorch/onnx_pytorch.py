...
class ConvertModel(nn.Module):
    ...
    def forward(self, *input_list, **input_dict):
        ...
        # if len(outputs) == 1:
        #     outputs = outputs[0]
        if (self.inter_name is not None) and self.is_inter:
            return self.inter_outputs, inter_out
        # elif len(self.inter_outputs) == 1:
        #     return list(self.inter_outputs.values())[0]
        else: 
